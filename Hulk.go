...
func main() {
	...
	go func() {
		fmt.Println("-- blackbird/root attack started --\n           Go!\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("In use               |\tzapros OK |\tzapros err")
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- blackbird/root attack finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
	...
	<-ctlc
	fmt.Println("\r\n-- blackbird/root attack stopped in user --        \n")
}
